...
from agent.agent import agent_chat, group_chat, group_synthesize
...
def group_chat_endpoint():
    ...
    AI 群聊 - 4个角色同时回复
    Request: { message, sessionId? }
    Response: { messages: [{persona, emoji, color, text}], trip }
    """
    data = request.get_json(silent=True) or {}
    user_message = data.get('message', '').strip()
    session_id = data.get('sessionId', 'default')
    ...
    result = group_chat(user_message, sess["group_history"])
